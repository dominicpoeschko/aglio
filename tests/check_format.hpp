...
void check_format(std::string_view s) {
    if constexpr(std::is_same_v<T, Types::Primitive>) {
        if constexpr(api == Api::Ostream) {
            ...
            expected += ss.str();
            expected
              += R"(, i16: -1234, i32: -123456, i64: -12345678901, u8: *, u16: 1234, u32: 123456, u64: 12345678901, f32: 3.14159, f64: 2.71828, flag: 1})";
            CHECK(s == expected);
        } else {
            std::string expected
              = R"({i8: -42, i16: -1234, i32: -123456, i64: -12345678901, u8: 42, u16: 1234, u32: 123456, u64: 12345678901, f32: 3.14159, f64: 2.718281828, flag: true})";
            CHECK(s == expected);
        }
        ...
        if constexpr(api == Api::Format || api == Api::Ostream) {
            std::string expected
              = R"({ns: 123456789ns, us: 987654us, ms: 12345ms, s: 3600s, min: 90min, hr: 24h})";
            CHECK(s == expected);
        } else {
            std::string expected
              = R"({ns: 123456789ns, us: 987654µs, ms: 12345ms, s: 3600s, min: 90min, hr: 24h})";
            CHECK(s == expected);
        }
        ...
        std::string expected = R"({color: 3, status: 1})";
        CHECK(s == expected);
    } else {
